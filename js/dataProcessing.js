...
function processAirportStandby(
  date,
  actualTimes,
...
function calculatePerDiem(
  ...
  newCalculationResults.perDiem +=
    restHours * SALARY_DATA[selectedRole].perDiemRate;
  ...
  if (flightNumbers.length === 1) {
    ...
    if (reportTime && debriefTime) {
      ...
      newParsedFlights.push({
        ...
        flight: duties,
        sector: details,
        reporting: reportTime || 0,
        debriefing: debriefTime || 0,
        hours: 0,
        pay: 0,
        isOutbound: true,
    ...
    for (let i = 0; i < flightNumbers.length; i++) {
      ...
      if (flightReportTime && flightDebriefTime) {
        ...
        newParsedFlights.push({
          ...
          flight: flightNumbers[i],
          sector: sectors[i] || "",
          reporting: flightReportTime || 0,
          debriefing: flightDebriefTime || 0,
          hours: 0,
          pay: 0,
          isTurnaround: true,
          isLayover: isLayover, // Mark as layover if identified as such
...
function processRosterData(rosterData, selectedRole) {
    ...
    if (typeof rosterData === "string") {
      ...
      try {
        ...
        if (parseResult.data && parseResult.data.length > 0) {
          console.log(
            "Successfully parsed string to array with",
      ...
      if (rosterData.data && Array.isArray(rosterData.data)) {
        console.log(
          "Roster data is a Papa Parse result object, extracting data array"
        );
  ...
  for (let i = 0; i < rosterData.length; i++) {
    if (rosterData[i] && rosterData[i][0] === "Schedule Details") {
      ...
      console.log(
        "Found Schedule Details at row",
        i,
        "starting processing from row",
        scheduleStartIndex
      );
      break;
    }
  ...
  if (scheduleStartIndex === 0) {
    console.log(
      "Schedule Details not found, looking for Date/Duties/Details header..."
    );
    for (let i = 0; i < rosterData.length; i++) {
      if (
        rosterData[i] &&
        ...
        console.log(
          "Found Date/Duties/Details header at row",
          i,
          "starting processing from row",
          scheduleStartIndex
        );
  ...
  if (scheduleStartIndex === 0) {
    console.error(
      "Could not find Schedule Details or Date/Duties/Details header in the roster data"
    );
  ...
  for (let i = scheduleStartIndex; i < rosterData.length; i++) {
    if (rosterData[i] && rosterData[i][0] === "Total Hours and Statistics") {
      scheduleEndIndex = i;
      console.log(
        "Found Total Hours and Statistics at row",
        i,
        "ending processing at this row"
      );
      break;
    }
  }
  ...
  for (let i = 0; i < Math.min(10, rosterData.length); i++) {
    for (let j = 0; j < rosterData[i].length; j++) {
      if (
        rosterData[i][j] &&
        typeof rosterData[i][j] === "string" &&
        rosterData[i][j].includes("/") &&
        rosterData[i][j].includes(" - ")
      ) {
  ...
  for (let i = scheduleStartIndex; i < scheduleEndIndex; i++) {
    ...
    if (row[0] && row[1]) {
      ...
      if (flightMonthNumber !== primaryMonthNumber) {
        ...
        console.log(
          `Excluding flight at row ${i} - belongs to month ${flightMonthNumber}, not primary month ${primaryMonthNumber}`
        );
        continue;
      }
      ...
      if (duties === "SBY") {
        ...
        const result = processAirportStandby(
          date,
          actualTimes,
          newCalculationResults,
          newParsedFlights,
        ...
        // Flight duty
        console.log(
          `Processing flight duty at row ${i}: ${duties} - ${details}`
        );
        flightDutyCount++;
        const result = processFlightDuty(
          date,
          duties,
          details,
          reportTime,
        ...
        newParsedFlights = result.newParsedFlights;
        console.log(
          `After processing flight duty, newParsedFlights length: ${newParsedFlights.length}`
        );
      } else {
    ...
    if (outboundFlight.sector && outboundFlight.sector.startsWith("DXB - ")) {
      ...
      for (let j = i + 1; j < newParsedFlights.length; j++) {
        ...
        if (returnFlight.sector === expectedReturnSector) {
          ...
          newCalculationResults = calculatePerDiem(
            outboundFlight,
            returnFlight,
            newCalculationResults,
            selectedRole
  ...
  if (newParsedFlights.length === 0) {
    ...
    if (basicSalary > 0) {
      console.log(
        "Returning calculation results with fixed salary components only"
      );
    } else {
